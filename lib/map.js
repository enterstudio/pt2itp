...
module.exports = function(argv, cb) {
    ...
    function mergeNames(names) {
        let bar = new prog('[:bar] :percent :etas Clustering Data', {
            complete: '=',
            incomplete: ' ',
            width: 20,
            total: names.address.length + names.network.length
        });

        let mergeQ = Queue();

        mergeQ.defer(next, 'address', 0);
        mergeQ.defer(next, 'network', 0);

        function next(type, it, done) {
             let matchQ = new Queue();

            for (let it_it = it; it_it < it+100; it_it++) {
                if (!names[type][it_it]) break;
                matchQ.defer(cluster[type], names[type][it_it], pool);
            }
            ...
            matchQ.await((err) => {
                ...
                if (it > names[type].length) return done();
                return next(type, it+100, done);
            });
        }
