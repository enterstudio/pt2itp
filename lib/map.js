...
module.exports = function(argv, cb) {
    ...
    function getNames() {
        ...
        nameQ.defer(function(done) {
            pool.query('SELECT text FROM network WHERE text IS NOT NULL GROUP BY text', function(err, res) {
        ...
        nameQ.defer(function(done) {
            pool.query('SELECT text FROM address WHERE text IS NOT NULL GROUP BY text', function(err, res) {
                if (err) return done(err);
    ...
    function matcher(calcFreq) {
        pool.connect(function(err, client, pg_done) {
            ...
            client.query(`SELECT count(*) FROM network_cluster WHERE text != '' AND TEXT IS NOT NULL`, (err, res) => {
                ...
                var cursor = client.query(new Cursor(`SELECT id FROM network_cluster WHERE text != '' AND TEXT IS NOT NULL;`));
                var batch = 0;
