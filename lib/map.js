...
module.exports = function(argv, cb) {
    ...
    if (argv.tokens) {
        let parsed =  JSON.parse(fs.readFileSync(path.resolve(__dirname, './tokens/', argv.tokens + '.json'), 'utf8'));
        let tokens = {};
                ...
                function iterate() {
                    cursor.read(100, (err, rows) => {
                        ...
                        rows.forEach((row) => {
                            ...
                            feat.properties['carmen:center'] = turf.pointOnSurface(feat.geometry.geometries[0]);
