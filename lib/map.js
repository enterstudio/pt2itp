...
module.exports = function(argv, cb) {
    ...
    function indexFeat() {
        ...
        indexQ.awaitAll(function(err, res) {
            ...
            getNames(addressDB, networkDB);
        });
    }

    function getNames(addressDB, networkDB) {
        var nameQ = Queue();
        ...
        nameQ.awaitAll(function(err, res) {
            ...
            mergeNames(names, dbs);
        });
    }

    function mergeNames(names, dbs) {
        var addressQ = Queue(1);

        for (var address_it = 0; address_it < names.address.length; address_it++) {
            addressQ.defer(cluster, names.address[address_it], dbs.address, 'address');
        }

        addressQ.await(function(err, res) {
            if (err) return cb(err);
        });
    }
}
