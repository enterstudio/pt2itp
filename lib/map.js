...
module.exports = function(argv, cb) {
        ...
        groupQ.await(function(err) {
            ...
            `, function(err, res) {
                ...
                pool.query(`SELECT id FROM network_cluster WHERE text != '';`, function(err, res) {
                    ...
                    matcher(res.rows, calcFreq);
                });
            });
        });
    }

    function matcher(networkIds, calcFreq) {
        var matchQ = Queue();

        for (var network_it = 0; network_it < networkIds.length; network_it++) {
            matchQ.defer(cluster.match, networkIds[network_it].id, calcFreq, pool);
        }
        ...
        matchQ.await(function(err) {
            ...
            pool.query(`SELECT id FROM network_cluster WHERE text != '' AND address IS NOT NULL;`, function(err, res) {
                if (err) return cb(err);
    ...
    function splitter(ids) {
        var splitQ = Queue();

        for (var it = 0; it < ids.length; it++) {
            splitQ.defer(split, ids[it].id, pool, output);
        }
