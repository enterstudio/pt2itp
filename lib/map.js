...
module.exports = function(argv, cb) {
    ...
    function createIndex(total) {
        ...
        index.init((err) => {
            ...
            indexQ.defer(index.copy, fs.createReadStream(path.resolve(__dirname, '..',  argv['in-address'])), 'address', {
                tokens: argv.tokens,
            ...
            indexQ.defer(index.copy, fs.createReadStream(path.resolve(__dirname, '..',  argv['in-network'])), 'network', {
                tokens: argv['tokens'],
                error: argv.error,
    ...
    function mergeNames(names) {
        ...
        mergeQ.defer(cluster.address);
        mergeQ.defer(cluster.network);
            ...
            client.query(`SELECT count(*) FROM network_cluster WHERE text != '' AND TEXT IS NOT NULL`, (err, res) => {
                ...
                function iterate() {
                    cursor.read(100, (err, rows) => {
                        ...
                        for (let row_it = 0; row_it < rows.length; row_it++) {
                            matchQ.defer(cluster.match, rows[row_it].id, calcFreq);
                        }
