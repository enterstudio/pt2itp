...
function split(id, pool, cb) {
    ...
    `, function(err, res) {
        ...
        for (var pt_it = 0; pt_it < feat.address.geometry.coordinates.length; pt_it++) {
            var pt = turf.point(feat.address.geometry.coordinates[pt_it]);

            for (var ln_it = 0; ln_it < feat.network.geometry.coordinates.length; ln_it++) {
                var ln = turf.lineString(feat.network.geometry.coordinates[ln_it]);
