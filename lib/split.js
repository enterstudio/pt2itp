...
function split(nid, cb) {
    ...
    `, (err, res) => {
        ...
        for (let it = 0; it < address.coordinates.length; it++) {
            let pt = turf.point(address.coordinates[it]);
        ...
        for (let it = 0; it < addressCluster.length; it++) {
            segs.push({
                address: turf.multiPoint(addressCluster[it]),
                network: turf.feature(network.features[it].geometry),
                number: numberCluster[it]
            });
        }

        potential = interpolize(text, segs, {
            debug: opts.debug
        });

        console.error('!! --- I AM FINISHED --- !!');
