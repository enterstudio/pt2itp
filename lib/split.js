...
function split(nid, cb) {
    ...
    `, (err, res) => {
        ...
        for (let pt_it = 0; pt_it < address.coordinates.length; pt_it++) {
            ...
            numberCluster[currentMatch.ln].push(number[pt_it]);
        }
        ...
        for (let it = 0; it < addressCluster.length; it++) {
            segs.push({
                address: turf.multiPoint(cluster[it]),
                network: turf.feature(network.features[pt_it].geometry),
                number: numberCluster[it]
            });
        }

        interpolize(text, segs, {
            debug: opts.debug
        });
