...
module.exports = function(data, xyz, writeData, done) {
    ...
    function itp(street_keys_it) {
        ...
        if (link.dups.reverse[streetID].length > 1) {
            split(streets, addresses.features[link.link[street_keys[street_keys_it]]], link.dups.reverse[streetID], xyz);
        }

        newStreet = interpolize(streets.features[street_keys[street_keys_it]], addresses.features[link.link[street_keys[street_keys_it]]], global.mapOptions);
