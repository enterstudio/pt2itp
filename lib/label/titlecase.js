...
module.exports = (opts) => {
    ...
    return (feature, synonym) => {
        synonym = !!synonym;

        let text, other;
        if (opts.favor && (opts.favor === 'network')) {
            text = feature.network_text.trim();
            other = feature.address_text.trim();
        }
        else {
            text = feature.address_text.trim();
            other = feature.network_text.trim();
        }
        ...
        if (synonym && (diacritics(text) !== diacritics(other)))
            return titleCase(text) + ',' + titleCase(other);
        else
