...
const termops = require('../node_modules/@mapbox/carmen/lib/util/termops');
...
function test(argv, cb) {
    ...
    pool.connect((err, client, pg_done) => {
        index.getMeta(pool, true, (err, meta) => {
                            ...
                            for (addr of row.geom.coordinates) {
                                ...
                                addrQ.defer((query, opts, done) => {
                                    ...
                                    c.geocode(query, opts, (err, res) => {
                                        ...
                                        if (!res.features.length) return done(null, `NO RESULTS|${query}|${opts.proximity.join(',')}`);

                                        let matched = termops.tokenize(res.features[0].place_name).join(' ');
                                        let cleanQuery = termops.tokenize(query).join(' ');

                                        if (matched !== cleanQuery) return done(null, `TEXT FAIL => ${res.features[0].place_name}|${query}|${opts.proximity.join(',')}`);
                                        else if (turf.distance(res.features[0], opts.proximity, 'kilometers') > 1) return done(null, `DIST FAIL|${query}|${opts.proximity.join(',')}`);
