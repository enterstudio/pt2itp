...
function test(argv, cb) {
    ...
    pool.connect((err, client, pg_done) => {
        index.getMeta(pool, 'units', (err, unitMap) => {
                            ...
                            for (addr of row.geom.coordinates) {
                                ...
                                addrQ.defer((query, opts, done) => {
                                    ...
                                    c.geocode(query, opts, (err, res) => {
                                        ...
                                        if (res.features[0].place_name.toLowerCase() !== query.toLowerCase()) return done(null, `TEXT FAIL|${query}|${opts.proximity.join(',')}`);
