    ...
    */
    copy(file, type, opts = {}, cb) {
        ...
        while (cpu_spawn--) {
            ...
            if (opts.error) child.stderr.pipe(process.stderr)//opts.error);
            ...
            child.on('exit', () => {
                console.error('ok - import child exited');
            });
            ...
            child.on('message', (message) => {
                if (opts.bar && typeof message === 'number') return opts.bar.tick(message);

                ready++;

                console.error(message)

                if (ready >= Math.floor(CPUS / 2)) copyRes();
            });
        }
        ...
        function copyRes() {
            console.error('ok - standardized input data')
            console.error('ok - importing data');

            if (type === 'address') {
                self.pool.query(`
                    COPY address (text, text_tokenless, _text, lon, lat, number)
                    FROM '${psv}' WITH CSV DELIMITER '|' QUOTE E'\b' NULL AS '';
                `, (err, res) => {
                    return cb(err);
                });
            } else {
