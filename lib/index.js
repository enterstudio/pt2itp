...
function index(pool, stream, type, opts, cb) {
    ...
    pool.connect(function(err, client, release) {
        ...
        if (type === 'address') {
            client.query('CREATE TABLE '+type+' (id SERIAL, text TEXT, _text TEXT, number INT, geom GEOMETRY(GEOMETRY, 4326));', function(err, res) {
                if (err) return cb(err);

                createRL(client, release);
            });
        } else {
            client.query('CREATE TABLE '+type+' (id SERIAL, text TEXT, _text TEXT, geom GEOMETRY(GEOMETRY, 4326));', function(err, res) {
                if (err) return cb(err);
