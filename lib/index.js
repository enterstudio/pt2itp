 ...
 * @param {Stream}      stream  of geojson Features to be indexed/imported
 * @param {Object}      opts    optional args
 * @param {Function}    cb      Callback
 * @return {Function}           in form fxn(err)
 */
function itp(pool, stream, opts = {}, cb) {
    let rl = readline.createInterface({
        input: stream,
        output: fs.createWriteStream(`${os.tmpDir()}/debug.psv`)
    });

    let errFile = fs.createWriteStream(`${os.tmpDir()}/debug-read.err`);
