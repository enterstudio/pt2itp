...
function init(pool, cb) {
    pool.connect((err, client, release) => {
        ...
        client.query(`
            ...
            CREATE TABLE meta (k TEXT UNIQUE, v JSON);
            COMMIT;
        `, (err, res) => {
            client.release();
            return cb(err);
...
function setMeta(pool, key, value, cb) {
    pool.query(`
        INSERT INTO meta (k, v)
            VALUES ('${key}', '${JSON.stringify(value)}')
            ON CONFLICT (k) DO
                UPDATE SET v = '${JSON.stringify(value)}';
    `, (err, res) => {
