...
function copy(pool, stream, type, opts = {}, cb) {
    ...
    rl.on('line', (line) => {
        ...
        try {
            ...
            if (argv.error) {
                argv.error.write(`Unable to parse or process\t${JSON.stringify(feat)}`);
            }
            feat = false;
        }
        ...
        if (!feat || typeof feat !== 'object') {
            if (argv.error) {
                argv.error.write(`Rejected by map module\t${JSON.stringify(feat)}`);
            }
            return;
        }
        if (!feat.properties.street) {
            if (argv.error) {
                argv.error.write(`Missing street name\t${JSON.stringify(feat)}`);
            }
            return;
        }
        ...
        if (type === 'address') {
            if (feat.properties.number === null) {
                if (argv.error) {
                    argv.error.write(`.number cannot be null\t${JSON.stringify(feat)}`);
                }
                return;
            }
