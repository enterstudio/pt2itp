...
function optimize(pool, cb) {
    pool.connect((err, client, release) => {
        ...
        client.query(`
            BEGIN;
            UPDATE address SET geom = ST_SetSRID(ST_GeomFromGeoJSON(geomtext), 4326);
...
function copy(pool, stream, type, opts = {}, cb) {
    const map = opts.map ? require(__dirname + '/../' + opts.map).map : false;
    ...
    rl.on('close', () => {
        pool.connect((err, client, release) => {
            ...
            let query;
            if (type === 'address') query = `COPY address (text, _text, geomtext, number) FROM '${os.tmpDir()}/address.psv' WITH CSV DELIMITER '|' QUOTE E'\b' NULL AS '';`;
            else query = `COPY network (text, _text, geomtext) FROM '${os.tmpDir()}/address.psv' WITH CSV DELIMITER '|' QUOTE E'\b' NULL AS '';`;

            client.query(String(query), (err, res) => {
                console.log(String(query), err, res)
                cb(err);
                client.release();
            });
        }); 
