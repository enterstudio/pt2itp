...
function index(pool, stream, type, opts, cb) {
    ...
    pool.connect(function(err, client, release) {
        ...
        if (type === 'address') {
            client.query(`
                BEGIN;
                CREATE TABLE ${type} (id SERIAL, text TEXT, _text TEXT, number INT, geom GEOMETRY(POINT, 4326));
                CREATE TABLE ${type}_cluster (id SERIAL, text TEXT, _text TEXT, number TEXT, geom GEOMETRY(MULTIPOINT, 4326));
                COMMIT;
            `, function(err, res) {
                if (err) return cb(err);
            ...
            client.query(`
                BEGIN;
                CREATE TABLE ${type} (id SERIAL, text TEXT, _text TEXT, geom GEOMETRY(LINESTRING, 4326));
                CREATE TABLE ${type}_cluster (id SERIAL, text TEXT, _text TEXT, geom GEOMETRY(MULTILINESTRING, 4326));
                COMMIT;
            `, function(err, res) {
                if (err) return cb(err);
