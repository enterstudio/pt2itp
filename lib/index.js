...
function copy(pool, stream, type, opts = {}, cb) {
    ...
    rl.on('line', (line) => {
        ...
        try {
            ...
            if (opts.error) {
                opts.error.write(`Unable to parse or process\t${JSON.stringify(feat)}\n`);
            }
            feat = false;
        }
        ...
        if (!feat || typeof feat !== 'object') {
            if (opts.error) {
                opts.error.write(`Rejected by map module\t${JSON.stringify(feat)}\n`);
            }
            return;
        }
        if (!feat.properties.street) {
            if (opts.error) {
                opts.error.write(`Missing street name\t${JSON.stringify(feat)}\n`);
            }
            return;
        }
        ...
        if (type === 'address') {
            if (feat.properties.number === null) {
                if (opts.error) {
                    opts.error.write(`.number cannot be null\t${JSON.stringify(feat)}\n`);
                }
                return;
            }
