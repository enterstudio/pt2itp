...
function interpolize(text, splits, argv = {}) {
    ...
    if (max !== -1 && min !== Infinity) return itps;

    let diff =  Math.pow(10, Math.ceil(Math.log10(max - min) - 1)); //Nearest 10^n diff

    if (itps[0].properties['carmen:lfromhn'])
    if (itps[0].properties['carmen:rfromhn'])
