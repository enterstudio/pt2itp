...
function interpolize(text, splits, argv = {}) {
    ...
    for (let split of splits) {
        ...
        const resFeat = genFeat(text, split, result);
    ...
    if (max !== -1 && min !== Infinity) return split(itps);
    let d = diff(max, min);

    //Drop Lower Values on L/R side to include more potential addresses ie 22 => 0
    if (itps[0].properties['carmen:lfromhn'] < itps[0].properties['carmen:ltohn'])
        itps[0].properties['carmen:lfromhn'] = dropLow(itps[0].properties['carmen:lfromhn'], d);
    else
        itps[0].properties['carmen:ltohn'] = dropLow(itps[0].properties['carmen:ltohn'], d);

    if (itps[0].properties['carmen:rfromhn'] < itps[0].properties['carmen:rtohn'])
        itps[0].properties['carmen:rfromhn'] = dropLow(itps[0].properties['carmen:rfromhn'], d);
    else
        itps[0].properties['carmen:rtohn'] = dropLow(itps[0].properties['carmen:rtohn'], d);

    //Raise Upper Values on L/R side
    let end = itps.length -1;
    if (itps[end].properties['carmen:lfromhn'] > itps[end].properties['carmen:ltohn'])
        itps[end].properties['carmen:lfromhn'] = raiseHigh(itps[end].properties['carmen:lfromhn'], d);
    else
        itps[end].properties['carmen:ltohn'] = raiseHigh(itps[end].properties['carmen:ltohn'], d);

    if (itps[end].properties['carmen:rfromhn'] > itps[end].properties['carmen:rtohn'])
        itps[end].properties['carmen:rfromhn'] = raiseHigh(itps[end].properties['carmen:rfromhn'], d);
    else
        itps[end].properties['carmen:rtohn'] = raiseHigh(itps[end].properties['carmen:rtohn'], d);
...
function genFeat(text, split, result) {
    let tiles = [];
    ...
    res.properties['carmen:parityl'] = [ result.lparity ];
    ...
    res.properties['carmen:parityr'] = [ result.rparity ];
