...
function interpolize(street, address, argv) {
    ...
    var tiles = [];
    tiles = cover.tiles(street.geometry, {min_zoom: argv.zoom, max_zoom: argv.zoom});

    var centre = turf.pointOnSurface(street.geometry);

    if (!centre || !verifyCenter(doc.properties["carmen:center"], tiles)) {
        centre = [ tiles[0][2] - tiles[0][0] / 2, tiles[0][3] - tiles[0][1] / 2 ];
    } else {
    ...
    street.properties = {
        ...
        "carmen:center": centre.geometry.coordinates,
