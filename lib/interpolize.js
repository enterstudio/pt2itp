...
function interpolize(street, address, argv) {
    ...
    for (var dist_it = 0; dist_it < distStart.length; dist_it++) {
        if (!result.lstart && distStart[dist_it].side === leftSide) {
            result.lstart = distStart[dist_it].number;
        } else if (!result.rstart && distStart[dist_it].side !== leftSide) {
            result.rstart = distStart[dist_it].number;
        }
    }

    for (var dist_it = 0; dist_it < distEnd.length; dist_it++) {
        if (!result.lend && distEnd[dist_it].side === leftSide) {
            result.lend = distEnd[dist_it].number;
        } else if (!result.rend && distEnd[dist_it].side !== leftSide) {
