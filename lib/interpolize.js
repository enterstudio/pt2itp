...
function interpolize(street, address, argv) {
    ...
    var tiles = [];
    tiles = cover.tiles(street.geometry, {min_zoom: 20, max_zoom: 20});

    var centre = turf.pointOnSurface(street.geometry);

    if (!centre || !verifyCenter(centre.geometry.coordinates, tiles)) {
        tmptiles = cover.tiles(street.geometry, {min_zoom: 20, max_zoom: 20}); //14 is the max resolution for carmen
