...
function closestCluster(str, addresses) {
    ...
    for (var addr_it = 0; addr_it < addresses.features.length; addr_it++) {
        ...
        for (var pt_it = 0; pt_it < addr.geometry.coordinates.length; pt_it++) {
            ...
            if (dist <= 0.100) { //I don't really care about addresses further away than this
                closest.push({
                    dist: dist,
    ...
    var sorter = {};
    for (var closest_it = 0; closest_it < closest.length; closest_it++) {
        var c = closest[closest_it];

        if (sorter[c.cluster]) {
            sorter[c.cluster].dist = sorter[c.cluster].dist + c.dist;
            sorter[c.cluster].tot++;
        } else {
            sorter[c.cluster] = { dist: c.dist, tot: 1 };
        }
    }

    var sorter_keys = Object.keys(sorter);
    sorter_keys.sort(function(a, b) {
        return (sorter[b].dist / sorter[b].tot) - (sorter[a].dist / sorter[a].tot);
    });

    var res = sorter_keys.map(function(sort) {
        return addresses.features[sort]
    });
