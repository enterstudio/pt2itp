...
function address(pool, cb) {
    pool.query(`
        ...
        COMMIT;
    `, (err, res) => {
        console.error('ok - clustered addreses');
...
function match(id, freq, pool, cb) {
    pool.query(`
        ...
        WHERE
            network.id = ${id} AND
            ST_Contains(ST_Buffer(ST_Envelope(network.geom), 0.01), addr.geom);
    `, (err, res) => {
...
function name(id, pool, cb) {
    pool.query(`SELECT ST_AsGeoJSON(geom) AS geom FROM network WHERE id = ${id}`, (err, res) => {
        ...
        pool.query(`SELECT text, _text FROM address WHERE ST_Within(geom, ST_SetSRID(ST_GeomFromGeoJSON('${JSON.stringify(buff)}'), 4326))`, (err, res) => {
            if (err) return cb(err);
