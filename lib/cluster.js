...
function closestCluster(str, addresses) {
    ...
    for (var closest_it = 0; closest_it < closest.length; closest_it++) {
        var c = closest[closest_it].cluster;

        if (sorter[c]) sorter[c]++;
        else sorter[c] = 1;
    }

    var sorter_keys = Object.keys(sorter);
    sorter_keys.sort(function(a, b) {
        return sorter[b] - sorter[a];
    });

    if (sorter_keys.length > 0) {
        var current = sorter[sorter_keys[0]];
        var res = [addresses.features[current]]
        for (var closest_it = 1; closest_it < closest.length; closest_it++) {
            if (current === sorter[sorter_keys[closest_it]]) continue;

            res.push(addresses.features[sorter[sorter_keys[closest_it]]]);
            break;
        }
