 ...
 *
 * @param {Array} streets       An array of tokenized street names ie: [[ 'main', 'st' ], ['fake', 'rd']]
 * @param {Array} addresses     An array of tokenized street names ie: [[ 'main', 'st' ], ['fake', 'rd']]
...
module.exports = function(streets, addresses) {
    ...
    var freq_keys = _.difference(Object.keys(nameFreq), ['_min', '_max']);
    ...
    function freq(feats) {
        for (var feat_it = 0; feat_it < feats.length; feat_it++) {
            for (var token_it = 0; token_it < feats[feat_it].length; token_it++) {
                var token = feats[feat_it][token_it];
                if (nameFreq[token]) {
                    nameFreq[token]++;
