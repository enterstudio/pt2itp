...
function map(feat) {
    ...
    if (!feat.geometry.coordinates instanceof Array || feat.geometry.coordinates.length !== 2) return false;
    if (isNaN(feat.geometry.coordinates[0]) || feat.geometry.coordinates[0] < -180 || feat.geometry.coordiantes[0] > 180) return false;
    if (isNaN(feat.geometry.coordinates[1]) || feat.geometry.coordinates[1] < -90 || feat.geometry.coordinates[1] > 90) return false;

    if (typeof feat.properties.number !== 'string') feat.properties.number = String(feat.properties.number);

    if (!/^\d+[a-z]?$/.test(feat.properties.number) && !/^(\d+)-(\d+)[a-z]?$/.test(feat.properties.number) !/^(\d+)([nsew])(\d+)[a-z]?$/.test(feat.properties.number)) return false;
