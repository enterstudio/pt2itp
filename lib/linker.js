...
module.exports = function(nameFreq, streets, addresses) {
    ...
    for (var street_it = 0; street_it < streets.features.length; street_it++) {
        var streetName = streets.features[street_it].properties.street.join(',');

        if (dups.reverse[streetName]) {
            dups.reverse[streetName].push(street_it);
            dups.forward[street_it] = streetName;
        } else { 
            dups.process.push(street_it);
            dups.reverse[streetName] = [street_it];
            dups.forward[street_it] = streetName;
        }
    }

    for (var toProcess_it = 0; toProcess_it < dups.process.length; toProcess_it++) {
        var street_it = dups.process[toProcess_it];
...
function getHighest(links, link, dups) {
    var highest = {
        street: null,
        addr: null,
        score: 0
    ...
    for (var street_it = 0; street_it < links.length; street_it++) {
        if (link[street_it] || !links[street_it]) continue;
    ...
    if (highest.score > 0) {
        ...
        return getHighest(links, link, dups)
    } else {
        return link;
    }
}
