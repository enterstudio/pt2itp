...
module.exports = (street, addresses) => {
    ...
    let debugId = false;
    if (process.env.LINKER_DEBUG) {
	if (parseInt(process.env.LINKER_DEBUG) === street.id) {
	    debugId = street.id;
	    console.error(`=== [linker] running for ${street.text} ===`);
	}
    }
	...
	if (address.text_tokenless && street.text_tokenless && (address.text_tokenless[0] !== street.text_tokenless[0]))
        ...
        //Calculate % Match
	console.error(address.text, street.text);
	console.error(address.text_tokenless, street.text_tokenless);
        let score = 100 - (((2 * levScore) / (address.text.length + street.text.length)) * 100)

        if (!current || current.score < score) {
	    if (debugId)
		console.error(`    - new leader: ${address.text}`);
            current = {
                score: score,
                address: address
            };
