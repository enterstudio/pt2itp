...
function start() {
    ...
    rl.on('line', (data) => {
        ...
        if (bytes > 10000) {
            process.send(bytes); //Update progress bar
            bytes = 0;
        }
        ...
        if (opts.type === 'address') {
            ...
            rl.output.write(`${str(feat.properties.street)}|${str(feat.properties.streetTokenless)}|${str(feat.properties._text)}|${JSON.stringify(str(feat.geometry.coordinates[0]))}|${JSON.stringify(str(feat.geometry.coordinates[1]))}|${str(feat.properties.number)}\n`);
        } else {
            rl.output.write(`${str(feat.properties.street)}|${str(feat.properties.streetTokenless)}|${str(feat.properties._text)}|${str(JSON.stringify(feat.geometry))}\n`);
        }
    });
}
...
function init(o) {
    ...
    console.error(path.resolve(__dirname, '..',  opts.file))
    opts.file = fs.createReadStream(path.resolve(__dirname, '..',  opts.file));
}
