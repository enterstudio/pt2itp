        ...
        if (streets.features[feat_it].geometry.type !== 'MultiLineString') {
            ...
            for (line_it = 0; line_it < streets.features[feat_it].geometry.coordinates.length; line_it++) {
                ...
                if (_.isEqual(working[0],coords[0]) && ( wBearing > 0 ? 
                    btn(sBearing + 180, wBearing + degTolerance, wBearing - degTolerance) :
                    btn(sBearing, wBearing + degTolerance, wBearing - degTolerance)
                )){
                    working = streets.features[feat_it].geometry.coordinates.splice(line_it, 1)[0].reverse().concat(working);
                    line_it = -1;

                // -c-> . -w-> && <-w- <-c-
                } else if (_.isEqual(working[0],coords[coords.length-1]) && ( wBearing < 0 ?
                    btn(eBearing + 180, wBearing + degTolerance, wBearing - degTolerance) :
                    btn(eBearing, wBearing + degTolerance, wBearing - degTolerance)
                )){
                    working = streets.features[feat_it].geometry.coordinates.splice(line_it, 1)[0].concat(working);
                    line_it = -1;

                // -w-> . -c-> && <-c- . <-w-
                } else if (_.isEqual(working[working.length-1],coords[0]) && ( wBearing < 0 ?
                    btn(sBearing + 180, wBearing + degTolerance, wBearing - degTolerance) :
                    btn(sBearing, wBearing + degTolerance, wBearing - degTolerance)
                )){
